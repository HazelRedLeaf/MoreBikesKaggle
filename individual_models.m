...
% main function
function [finalMAE] = individual_models(num)
...
%         'full_profile_3h_diff_bikes'};
    ...
    interests = {'hour','weekday','weekhour', ...
        'windMaxSpeedms','windMeanSpeedms', ...
        'windDirectiongrades','temperatureC','relHumidityHR', ...
        'airPressuremb','bikes_3h_ago','full_profile_bikes', ...
        'full_profile_3h_diff_bikes'};
    ...
    [a_ls, y_estimate, y, X, baselineMAE, firstMAE] = least_squares(dataTable, interests, feature_index, number_of_features);
    fprintf('a_ls = %d\n', a_ls);
    
    % produce a new, more accurate model without the outliers
    [a_ls, finalMAE] = ls_no_outliers(y, y_estimate, X);
    fprintf('a_ls = %d\n', a_ls);
...
function[a_ls, y_estimate, y, X, baselineMAE, firstMAE]=least_squares(dataTable, interests, feature_index , number_of_features)
    ...
    % 'bikes'
    firstMAE = mean(abs(y - y_estimate));
...
% use the features ranked and 'bikes' to compute least squares and predict 
% estimated 'bikes'; calculate MAE between 'bikes' and 'bikes'-estimate
function[a_ls, MAE] = ls_no_outliers(y, y_estimate, X)
    % for each chosen feature
    % remove NaN rows
    % do least squares prediction for bikes
    % calculate MAE for bikes based
